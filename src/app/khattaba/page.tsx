...
import {
  ...
  Workflow, Send, KeyRound, Phone, Mail, Wallet,
...
export default function KhattabaScopeDoc() {
  ...
  const publicPages: { code: string; page: string; desc: string; icon: LucideIcon }[] = [
    ...
    { code: "P02", page: "من نحن", desc: "نبذة عن المؤسسة ورؤيتها ورسالتها والسجل التجاري والتراخيص", icon: FileText },
  ...
  const journey: { step: number; phase: string; desc: string; icon: LucideIcon }[] = [
    { step: 1, phase: "الوصول", desc: "يدخل المستخدم الموقع أو التطبيق ويضغط (تسجيل جديد)", icon: Globe },
    { step: 2, phase: "التسجيل", desc: "يملأ نموذج التسجيل (الاسم، الجنس، العمر، الجنسية، المدينة، المؤهل، الحالة الاجتماعية، الوظيفة، وصف مختصر)", icon: UserPlus },
    { step: 3, phase: "التحقق", desc: "يستلم رمز OTP على جواله ويدخله للتحقق", icon: KeyRound },
    { step: 4, phase: "التفعيل", desc: "يتم تفعيل الحساب فوراً بعد إدخال رمز OTP الصحيح", icon: CheckCircle2 },
    { step: 5, phase: "التصفح والبحث", desc: "يتصفح بروفايلات الطرف الآخر ويستخدم الفلاتر للبحث", icon: Search },
    { step: 6, phase: "طلب التواصل", desc: "يضغط (أرغب في التواصل) على بروفايل معين", icon: Send },
    { step: 7, phase: "الإشعار", desc: "يتم إرسال إشعار (Push + SMS) للطرف الآخر بوجود طلب تواصل", icon: Bell },
    { step: 8, phase: "المراجعة", desc: "الطرف الآخر يراجع البروفايل ويقرر (قبول أو رفض)", icon: Eye },
    { step: 9, phase: "الدفع", desc: "في حال القبول: يُطلب من الطرفين الدفع عبر بوابة الدفع", icon: CreditCard },
    { step: 10, phase: "الشات المراقب", desc: "تُفتح غرفة شات مراقبة لمدة 15 دقيقة (نص فقط)، مع فلترة تلقائية", icon: MessageCircle },
    { step: 11, phase: "التوصيل عبر الإدارة", desc: "بعد انتهاء المدة أو بطلب الطرفين، يظهر زر (تواصل مع الإدارة عبر واتساب) مع كود خاص بكل طرف", icon: Workflow },
    { step: 12, phase: "إتمام التوصيل", desc: "الإدارة تتواصل مع الطرفين وتوصلهم ببعض بطريقة آمنة وشرعية", icon: Heart },
  ];
  ...
  const chatEnd: { title: string; desc: string; color: string; icon: LucideIcon }[] = [
    { title: "انتهاء المدة", desc: "بعد 15 دقيقة تُغلق غرفة الشات تلقائياً ويظهر للطرفين زر التواصل مع الإدارة.", color: A, icon: Clock },
    { title: "طلب التوصيل", desc: "أي من الطرفين يضغط (أرغب في التوصيل عبر الإدارة) ويتم إنهاء الشات وتحويلهم.", color: G, icon: ArrowRight },
    { title: "الإبلاغ", desc: "أي من الطرفين يضغط (إبلاغ) ويتم تجميد المحادثة وإشعار الأدمن فوراً.", color: R, icon: XCircle },
  ];

  const paymentQuestions: { q: string; note?: string }[] = [
    { q: "كم سعر طلب التواصل الواحد بالريال السعودي؟", note: "يُحدد من لوحة التحكم" },
    { q: "هل يوجد نظام اشتراك شهري بالإضافة لرسوم التواصل؟", note: "مثال: اشتراك شهري 99 ريال يتضمن X طلبات مجانية. يمكن تفعيله من لوحة التحكم" },
    { q: "في حال رفض الطرف الآخر لطلب التواصل، ما السياسة؟", note: "استرداد المبلغ، رصيد داخلي، أو لا استرداد. يمكن ضبطه من لوحة التحكم" },
    { q: "ما سياسة الاسترداد العامة للمشتركين؟", note: "مقترح: استرداد خلال 14 يوماً من الدفع لو لم تُستخدم الخدمة" },
    { q: "آلية الدفع داخل تطبيق الجوال: Web Payments أم In-App Purchases؟", note: "Apple تشترط في بعض الحالات استخدام IAP لمشتريات الخدمات الرقمية. اختيار المسار يحدد نسبة العمولة (Apple وGoogle تأخذان من 15 إلى 30% على IAP) ويؤثر على مراجعة المتجر، ويحتاج قراراً من العميل قبل بدء التطوير" },
  ];
  ...
  const operationalTerms: {
    ...
    },
    {
      ...
      body: "حساب Apple Developer ($99/سنة) وحساب Google Play Console ($25 لمرة واحدة): على العميل فتحهما وتسجيل بياناته الرسمية، ويتم نشر التطبيق من خلالهما باسم المؤسسة.",
      badge: "على العميل",
      badgeColor: B,
      icon: Smartphone,
    },
  ...
  const additionalFeatures: AdditionalFeature[] = [
    ...
    },
    {
      ...
      sections: [
        {
          subtitle: "عند تعديل البيانات",
          bullets: [
            {
              text: "أي تعديل يقوم به العضو على:",
              sub: ["الرغبات", "الشروط", "المعلومات الشخصية"],
            },
            "لا يتم نشره مباشرة.",
          ],
        },
          ...
          bullets: [
            {
              ...
              sub: ["لا ينشر مباشرة.", "يتم مراجعته من الإدارة."],
            },
          ],
        },
        {
          ...
          bullets: [
            {
              text: "يحق للإدارة:",
              sub: ["تعديل جميع الحقول.", "اعتماد أو رفض الطلب."],
            },
          ],
        },
        {
          ...
          bullets: [
            ...
            "يطلب منه سداد رسوم النشر التي يتم تحديدها من طرف إدارة المنصة.",
          ],
        },
        {
          subtitle: "مدة السداد",
    ...
    },
    {
      ...
      sections: [
        {
          subtitle: "دعم",
          bullets: ["Apple Pay", "التحويل البنكي", "وسائل الدفع الأخرى"],
        },
      ],
    },
    {
  ...
  const declarationNotes: string[] = [
    ...
    "تقوم الإدارة بوضع الأتمتة المناسبة لطباعة هذه التعهدات رسمياً وإرفاق شعار وبيانات المؤسسة واعتمادها كسندات تنظيمية وإدارية متى ما دعت الحاجة للرجوع إليها.",
  ...
  const womanFields: RegField[] = [
    ...
    },
    {
      ...
      note:
        "الزواج بشروط يتفق عليها الطرفين كل حسب ظروفه مثل عدم الالتزام بالمبيت اليومي أو توفير سكن أو نفقة وأي شروط أخرى يتفق عليها الطرفين، ويمكن تسجيل الزواج في أبشر أو خارج أبشر. (كتابة 500 حرف)",
    },
  ];
  ...
  return (
    <ArabicTailProcessor>
      <div ref={ref} style={{ background: "#fff", color: D, fontFamily: "'Ahmed Sans', sans-serif" }}>
          ...
          <div className="relative z-10 w-full max-w-5xl mx-auto px-6 flex flex-col items-center">
            ...
            <div className="ph-hero opacity-0 grid grid-cols-1 md:grid-cols-2 gap-4 mb-10 w-full max-w-2xl">
              ...
              >
                ...
                <p className="text-[14px] font-bold ar-body">مؤسسة خطابة السعودية الأولى للتجارة</p>
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 ph-stagger">
              {[
                ...
                { label: "العميل", val: "مؤسسة خطابة السعودية الأولى للتجارة (سجل تجاري: 1010158509)", icon: FileSignature, color: B },
        ...
        <section className="ph-slide opacity-0" style={{ padding: "100px 24px", background: "#fff", borderTop: "1px solid #F0F0F0" }}>
          <div className="max-w-5xl mx-auto">
            <SectionHead eyebrow="4.2 · قلب النظام" subtitle="غرفة محادثة آمنة محدودة الزمن مع فلترة محتوى تلقائية وتدخل إداري لحظي">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10 ph-stagger">
              {[
                { title: "مدة المحادثة", val: "15 دقيقة", desc: "تُحسب من لحظة فتح غرفة الشات بعد الدفع", icon: Clock, color: G },
                { title: "نوع المحتوى", val: "نصي فقط", desc: "لا يُسمح بإرسال صور أو ملفات أو رسائل صوتية", icon: FileText, color: B },
                { title: "المراقبة", val: "Live من الأدمن", desc: "إمكانية القراءة والتدخل الفوري في أي محادثة جارية", icon: Eye, color: P },
              ].map((c) => (
                <div
                  key={c.title}
        ...
        <section className="ph-slide opacity-0" style={{ padding: "100px 24px", background: "#fff", borderTop: "1px solid #F0F0F0" }}>
          <div className="max-w-4xl mx-auto">
            <SectionHead
              ...
              subtitle="أربعة قرارات يحددها العميل قبل الإطلاق · جميعها قابلة للتعديل لاحقاً من لوحة التحكم دون أي تعديلات برمجية"
            >
              نقاط <span style={{ color: G }}>تتطلب قراراً</span>
            </SectionHead>

            <div className="flex flex-col gap-4 ph-stagger">
              {paymentQuestions.map((q, i) => (
                <div
                  key={q.q}
                  className="ph-item rounded-[18px] p-6"
                  style={{ background: "#fff", border: "1px solid #EBEBEB", boxShadow: "0 1px 12px rgba(0,0,0,0.03)" }}
                >
                  <div className="flex items-start gap-4">
                    <span
                      className="ar-heading text-3xl flex-shrink-0"
                      style={{ color: `${G}50` }}
                    >
                      Q{i + 1}
                    </span>
                    <div className="flex-1">
                      ...
                      {q.note && (
                        <p className="text-[12px] ar-body leading-relaxed" style={{ color: "rgba(0,0,0,0.5)" }}>
                          {q.note}
                        </p>
                      )}
                    </div>
            ...
            >
              ...
              <p className="text-[12px] ar-body leading-relaxed" style={{ color: "rgba(0,0,0,0.7)" }}>
                ...
                جميع هذه الخيارات ستكون قابلة للتعديل من لوحة التحكم الإدارية، مما يمنح العميل مرونة كاملة في تحديد نموذج الأعمال دون الحاجة لتعديلات برمجية.
              </p>
            </div>
          </div>
        </section>
        ...
        <section id="section-09" className="ph-slide opacity-0" style={{ padding: "100px 24px", background: "#fff", borderTop: "1px solid #F0F0F0" }}>
          <div className="max-w-5xl mx-auto">
            <SectionHead
              ...
              subtitle="عشرون متطلباً وميزة تفصيلية مطلوبة في المنصة والتطبيق كما وردت في وثيقة العميل"
            >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-12">
              ...
              >
                ...
                <p className="ar-heading text-lg mb-6" style={{ color: D }}>
                  مؤسسة خطابة السعودية الأولى للتجارة
                </p>
                <div
        ...
        <section className="ph-slide opacity-0" style={{ padding: "60px 24px 80px", background: "#fff" }}>
          <div className="max-w-3xl mx-auto text-center">
            ...
            <p className="text-[11px] ar-body" style={{ color: "rgba(0,0,0,0.2)" }}>
              الملحق (أ) · الإصدار 1.0 · سري وخاص · &copy; {new Date().getFullYear()} مُعد لمؤسسة خطابة السعودية الأولى للتجارة
            </p>
          </div>
        </section>
      </div>
