...
const floatingPills = [
  ...
  { icon: SiReact, label: "تطوير الواجهات", color: "#111", bg: "#61DAFB", top: "80%", right: "1%", rotate: -6 },
];
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          ...
          <div className="about-anim opacity-0 w-full max-w-[320px] rounded-[32px] overflow-hidden mb-10" style={{ background: "#fff", border: "2px solid #004D5A", boxShadow: "6px 6px 0px 0px #004D5A" }}>
            ...
            <div className="relative">
              ...
              {/* Available */}
              <div className="absolute top-4 right-4 flex items-center gap-1.5 px-3 py-1.5 rounded-full" style={{ background: "rgba(4,50,58,0.5)", backdropFilter: "blur(10px)" }}>
                <span className="w-2 h-2 rounded-full" style={{ background: "#CFF7EE" }} />
                <span className="ar-body text-[10px] font-bold" style={{ color: "#CFF7EE" }}>متاح</span>
              </div>
              {/* Stars */}
              <div className="absolute top-4 left-4 flex items-center gap-0.5 px-2.5 py-1.5 rounded-full" style={{ background: "rgba(4,50,58,0.5)", backdropFilter: "blur(10px)" }}>
                {[1,2,3,4,5].map(i => <Star key={i} size={10} fill="#CFF7EE" style={{ color: "#CFF7EE" }} />)}
              </div>
              {/* Name on photo */}
              <div className="absolute bottom-4 right-5 left-5 text-center">
                <h3 className="ar-heading text-xl" style={{ color: "#fff" }}>أحمد علي</h3>
                <p className="ar-body text-xs" style={{ color: "#CFF7EE" }}>استراتيجي رقمي شامل</p>
              </div>
            </div>
            ...
            <div className="px-5 pb-5 pt-3">
              ...
              <div className="flex items-center justify-center gap-1.5 mb-4">
                ...
                <span className="ar-body text-xs" style={{ color: "#5B7B84" }}>جدة، السعودية</span>
              </div>
              {/* Mini stats */}
              <div className="flex items-center justify-between mb-4 px-1">
                {[{ v: "5+", l: "سنوات" }, { v: "50+", l: "مشروع" }, { v: "4", l: "دول" }].map((s, i, arr) => (
                  <div key={s.l} className="flex-1 text-center relative">
                    <p className="ar-heading text-lg font-bold" style={{ color: "#04323A" }}>{s.v}</p>
                    <p className="ar-body text-[9px] font-medium uppercase tracking-wider" style={{ color: "rgba(4,50,58,0.55)" }}>{s.l}</p>
                    {i < arr.length - 1 && (
                      <div className="absolute left-0 top-1/2 -translate-y-1/2 h-8 w-px" style={{ background: "rgba(0,77,90,0.12)" }} />
                    )}
                  </div>
                ))}
              </div>
              {/* CTA */}
              <a href="#contact" className="ar-body flex items-center justify-center gap-2 w-full h-11 rounded-full text-sm font-bold" style={{ background: "#CFF7EE", color: "#04323A", border: "2px solid #004D5A", boxShadow: "4px 4px 0px 0px #004D5A" }}>
