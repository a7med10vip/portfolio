...
/* Arabic mirror of the English marquee: two bands tilted opposite ways so they
   cross at the middle of the section — an X, not two parallel lines. */
const TILT = 7;
...
function Band({
  ...
  return (
    <div
      ...
      /* dir="ltr" because the strip is a flex row driven by a negative
         translate — under the page's RTL the axis flips, the animation runs
         against the order and the band empties out mid-loop. The words shape
         correctly regardless; only the order of the spans is pinned. */
      dir="ltr"
      style={{
      ...
      >
        {doubled.map((s, i) => (
          <span key={i} className="contents">
            <span
              className="ar-heading shrink-0 text-base md:text-xl whitespace-nowrap"
              /* Arabic sits taller in the line box than the Latin build's 1 */
              style={{ color, fontWeight: 400, lineHeight: 1.6 }}
            >
              {s}
            </span>
