...
const NUDGE_MESSAGES = [
  ...
  "لا تتردد, يمكنني إخبارك عن مشاريعي أو مهاراتي أو توفري.",
  "هل تودّ معرفة شيء ما؟ أنا على بُعد رسالة واحدة.",
];

export default function AskAhmedAr() {
    ...
    if (!hasWelcomed) {
      ...
      setTimeout(() => {
        setMessages([
          {
            role: "assistant",
            content: "أهلاً! 👋 مرحباً بك في معرض أعمالي. أنا أحمد, ما اسمك لأتمكن من مساعدتك بشكل أفضل؟",
          },
        ]);
        playNotification();
      }, 300);
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      const reply = data.reply || "تواصل معي على hello@ahmedali.online";
      setMessages((prev) => [...prev, { role: "assistant", content: reply }]);
      playNotification();
    } catch {
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: "تعذّر الاتصال. تواصل مع أحمد على hello@ahmedali.online" },
      ]);
    } finally {
      setIsTyping(false);
    }
    ...
    <>
      ...
      {!isOpen && (
        <div className="fixed bottom-7 right-7 z-[97] flex flex-col items-start gap-3">
          ...
          {(showGreeting || previewMsg) && (
            ...
            >
              {previewMsg ? (
                ...
                <>
                  اسألني عن أعمال أحمد ومهاراته وخبراته{" "}
                  <span style={{ display: "inline-block", animation: "askahmedar-wave 1s infinite", transformOrigin: "70% 70%", fontSize: "16px" }}>👋</span>
                </>
              )}
            </div>
          )}
          ...
          <a
            ...
            onClick={() => {
              import("@/lib/analytics").then((m) => m.trackEvent("whatsapp_click", { source: "floating_button", lang: "ar" }));
            }}
            style={{
              width: "48px",
              height: "48px",
            ...
            {unreadCount > 0 && (
              <span
                style={{
                  ...
                  color: "#fff",
                  zIndex: 2,
                  animation: "askahmedar-badge-pop 0.3s cubic-bezier(0.34,1.56,0.64,1)",
                }}
              >
                {unreadCount}
              </span>
      ...
      >
        ...
        >
          {messages.map((msg, i) => (
            <div
              ...
              style={{
                ...
                animation: "askahmedar-msg-in 0.3s ease forwards",
              }}
            >
              {msg.role === "assistant" ? (
          ...
          {isTyping && (
            <div style={{ display: "flex", gap: "10px", alignItems: "flex-end" }}>
              ...
              >
                <span className="askahmedar-dot" style={{ animationDelay: "0s" }} />
                <span className="askahmedar-dot" style={{ animationDelay: "0.2s" }} />
                <span className="askahmedar-dot" style={{ animationDelay: "0.4s" }} />
              </div>
            </div>
          )}
        </div>
        ...
        {showQuickReplies && messages.length <= 1 && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", padding: "0 16px 12px", background: "#FAFAFA", direction: "rtl" }}>
      ...
      <style>{`
        @keyframes askahmedar-badge-pop {
          from { transform: scale(0); }
          to { transform: scale(1); }
        }
        @keyframes askahmedar-wave {
          0%, 100% { transform: rotate(0deg); }
          25% { transform: rotate(20deg); }
          75% { transform: rotate(-10deg); }
        }
        @keyframes askahmedar-float-in {
          from { opacity: 0; transform: scale(0.8) translateY(10px); }
          to { opacity: 1; transform: scale(1) translateY(0); }
        }
        @keyframes askahmedar-msg-in {
          from { opacity: 0; transform: translateY(8px); }
          to { opacity: 1; transform: translateY(0); }
        }
        @keyframes askahmedar-typing {
        ...
        .askahmedar-dot {
          ...
          animation: askahmedar-typing 1.2s infinite;
        }
        .askahmedar-window::-webkit-scrollbar { width: 3px; }
        .askahmedar-window::-webkit-scrollbar-thumb { background: #E4E4E7; border-radius: 3px; }
