...
import { SECTIONS } from "@/app/fodi-cgm/data";
import { P as S, PD as D, LINE } from "./theme";
 ...
 */
export default function FodiNav() {
  ...
  useEffect(() => {
    ...
    try {
      const saved = localStorage.getItem("fodi-nav-pos");
  ...
  useEffect(() => {
    try { if (localStorage.getItem("fodi-nav-hint") === "1") return; } catch {}
    const t = window.setTimeout(() => setHint(true), 2600);
    const t2 = window.setTimeout(() => {
      setHint(false);
      try { localStorage.setItem("fodi-nav-hint", "1"); } catch {}
    }, 12000);
  ...
  useEffect(() => {
    ...
    const up = () => {
      ...
      offset.current = null;
      setPos((p) => {
        if (p) { try { localStorage.setItem("fodi-nav-pos", JSON.stringify(p)); } catch {} }
        return p;
      });
    };
      ...
      <div dir="rtl" className="fixed top-0 left-0 right-0 z-[94] lg:hidden"
        ...
        <button onClick={() => setOpen((o) => !o)}
          ...
          <span className="grid place-items-center rounded-lg shrink-0" style={{ width: 30, height: 30, background: `${colour}14` }}>
            <FaDroplet size={13} color={colour} />
          </span>
          <span className="flex-1 min-w-0">
