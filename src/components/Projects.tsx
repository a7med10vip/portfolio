...
/* Same named ramp as Services and How I Work. This section had been mixing
   three unrelated systems: the site's teal, black at 8/10/45/50% alpha, and a
   neutral grey pair (#F4F4F5 / #E4E4E7 / #e0e0e0) from a palette the site
   never adopted. */
const INK = "#04323A";
const TEAL = "#004D5A";
const MUTED = "#4E717A";
...
/* Badges over the screenshot share one recipe so the three corners match. */
...
const toolLogos: Record<string, { icon: string; color: string }> = {
  ...
  "Next.js": { icon: "/ext/nextjs.svg", color: "#04323A" },
  "Booking": { icon: "/ext/flaticon-seo.png", color: "#04323A" },
  "SEO Strategy": { icon: "/ext/semrush.png", color: "#FF642D" },
  "Real Estate": { icon: "/ext/google-ads-icon.png", color: "#4285F4" },
  "Content": { icon: "/ext/flaticon-social.png", color: "#04323A" },
...
export default function Projects() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      /* Desktop: the cards stack. Each one slides up over the one before it
         while that card settles back a little, so the deck reads as depth
         rather than a list scrolling past. */
      mm.add("(min-width: 1024px)", () => {
        const cards = gsap.utils.toArray<HTMLElement>(".proj-item");
        ...
        cards.slice(1).forEach((card, i) => {
          ...
          /* The outgoing card gives up a little scale and lifts, which is what
             sells the new one as being in front of it. */
          if (previous) tl.to(previous, { scale: 0.96, y: -18 }, i);
          tl.to(card, { yPercent: 0, scale: 1, y: 0 }, i);
        });
      });

      /* Below lg the cards are a plain column again — eight absolutely
         positioned cards in a pinned viewport has nowhere to go on a phone. */
    ...
    <section ref={sectionRef} id="projects" style={{ background: "#fff", padding: "100px 0" }}>
      <div className="max-w-[1400px] mx-auto px-6">
        ...
        {/* Projects deck. On desktop the outer block reserves the scroll the
            stack needs and the inner block pins; below lg both collapse and the
            cards fall back to an ordinary column. */}
        <div
          ref={stackRef}
            ...
            return (
              ...
              >
                <div className={`flex flex-col h-full ${isMint ? "lg:flex-row" : "lg:flex-row-reverse"}`} style={{ minHeight: "500px" }}>
                  {/* Image side. The letterbox behind the screenshot is the
                      site's ink, not the off-palette #161616 it used to be. */}
                  <div className="flex-shrink-0 relative overflow-hidden lg:h-full" style={{ background: INK }}>
                    {/* Neither cropped nor letterboxed: the image is a normal
                        block, so the panel inherits its aspect ratio. Full
                        width in the stacked layout, full card height at lg
                        with the panel widening to match. */}
                    {project.image && (
                      <img
                        src={project.image}
                        alt={project.title}
                    ...
                    {/* All three badges sit on a light fill with the retro
                        outline. The year and country badges used to be dark
                        teal type on a black 50% scrim — the text was there,
                        it just could not be read. */}
                    <div className="absolute top-5 left-5 px-3 py-1.5 rounded-full text-[11px] font-bold" style={badge("#fff")}>
                      {project.year}
                    </div>

                    {flag && (
                      <div className="absolute top-5 right-5 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-semibold" style={badge("#fff")}>
                        <span className="text-base">{flag.flag}</span>
                        {flag.name}
                      </div>
                    )}

                    <div className="absolute bottom-5 left-5 px-3 py-1.5 rounded-full text-[11px] font-semibold" style={badge(MINT)}>
                      {project.category}
                    </div>
                  </div>
                  ...
                  <div className="flex-1 min-w-0 p-8 md:p-10 flex flex-col justify-between" style={{ color: cardText }}>
                    <div>
                      ...
                      {/* Results — retro chips, same outline and hard shadow
                          as the Services tools and the How I Work highlights.
                          These were #F4F4F5 on #E4E4E7, a neutral grey pair
                          borrowed from a palette the site does not use. */}
                      ...
                      {/* Tool logos — the tiles took a #e0e0e0 border, a third
                          grey again unrelated to the palette. */}
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-6">
                      {project.link && (
                        <a href={project.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 h-10 px-6 rounded-full text-xs font-bold" style={{ background: isMint ? "#fff" : MINT, color: INK, border: `2px solid ${TEAL}`, boxShadow: `3px 3px 0px 0px ${TEAL}` }}>
                          Visit Live <ExternalLink size={12} />
                        </a>
                      )}
                      {(project as any).playStore && (
                        <a href={(project as any).playStore} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 h-11 px-4 rounded-xl" style={{ background: "#fff", border: `1.5px solid ${TEAL}`, boxShadow: `2px 2px 0px 0px ${TEAL}` }}>
                          ...
                          <div className="text-left"><span className="block text-[8px] leading-none" style={{ color: MUTED }}>GET IT ON</span><span className="block text-sm font-semibold leading-tight" style={{ color: INK }}>Google Play</span></div>
                        </a>
                      )}
                      {(project as any).appStore && (
                        <a href={(project as any).appStore} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 h-11 px-4 rounded-xl" style={{ background: "#fff", border: `1.5px solid ${TEAL}`, boxShadow: `2px 2px 0px 0px ${TEAL}` }}>
                          ...
                          <div className="text-left"><span className="block text-[8px] leading-none" style={{ color: MUTED }}>Download on the</span><span className="block text-sm font-semibold leading-tight" style={{ color: INK }}>App Store</span></div>
                        </a>
                      )}
                    </div>
                  </div>
        ...
        <div className="text-center mt-14">
          <a href="#contact" className="inline-flex items-center gap-2 h-12 px-8 rounded-full text-sm font-bold" style={{ background: MINT, color: INK, border: `2px solid ${TEAL}`, boxShadow: `4px 4px 0px 0px ${TEAL}` }}>
