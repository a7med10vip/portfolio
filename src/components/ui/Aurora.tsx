...
export default function Aurora({ colorStops = ["#FF4D00", "#FF8952", "#FF4D00"], amplitude = 1.0, blend = 0.5, speed = 1.0 }: AuroraProps) {
  ...
  useEffect(() => {
    ...
    import("ogl").then(({ Renderer, Program, Mesh, Color, Triangle }) => {
      ...
      const stopsArr = colorStops.map((hex) => { const c = new Color(hex); return [c.r, c.g, c.b]; });
      ...
      const update = (t: number) => {
        ...
        program.uniforms.uColorStops.value = (p.colorStops ?? colorStops).map((hex: string) => { const c = new Color(hex); return [c.r, c.g, c.b]; });
      ...
      return () => {
        ...
        if (ctn && (gl.canvas as HTMLCanvasElement).parentNode === ctn) ctn.removeChild(gl.canvas as HTMLCanvasElement);
