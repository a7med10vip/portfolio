  ...
  return (
    ...
    >
      {/* The dot grid that used to sit here was white dots at 3% opacity on a
          near-white ground — it rendered as nothing, so it is gone rather than
          kept as decoration that never arrives. */}

      <div className="max-w-6xl mx-auto relative">
        <div className="text-center mb-16">
          <p className="script text-xl md:text-2xl mb-3" style={{ color: TEAL }}>Impact</p>
          <h2 className="heading text-3xl md:text-4xl" style={{ color: INK }}>By The Numbers</h2>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 md:gap-6">
          {stats.map(({ label, prefix, suffix, description, Icon }, i) => (
            <div
              key={label}
              className="stat-card opacity-0 rounded-[20px] p-6 md:p-8 flex flex-col items-center text-center"
              style={{
            ...
            >
              ...
              <p className="text-sm font-bold mt-3" style={{ color: INK }}>
                {label}
              </p>
              <p className="text-[11px] mt-1" style={{ color: i % 2 === 0 ? "rgba(4,50,58,0.62)" : MUTED }}>
                {description}
              </p>
            </div>
          ))}
