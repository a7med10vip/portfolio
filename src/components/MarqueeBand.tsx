      ...
      <div className={`flex gap-5 shrink-0 ${animClass}`} style={{ willChange: "transform" }}>
        {doubled.map((s, i) => (
          <span key={i} className="contents">
            <span className="shrink-0 text-base md:text-lg px-5 md:px-6 py-2 md:py-2.5 rounded-full whitespace-nowrap" style={{ background: "#4FFFB0", color: "#0A0A0A", fontFamily: "'TAN Headline'", fontWeight: 400 }}>{s}</span>
...
export default function MarqueeBand() {
  return (
    <div className="relative" style={{ background: "#0A0A0A", padding: "60px 0", overflow: "hidden" }}>
      <style>{`
